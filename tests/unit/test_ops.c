...
static const struct netdev_ops unix_socket_ops;   /* 前置声明：init 中校验分派用 */
...
static int unix_socket_init(struct netdev *self)
{
    ...
    if (self->ops != &unix_socket_ops)
        return -1;
    self->name = "unix_socket_netdev";
    return 0;
}
...
static const struct netdev_ops unix_socket_ops = {   /* C99 指定初始化，static const */
...
static int loopback_start(struct netdev *self)
{
    (void)self;
    return 0;
}
...
static void loopback_stop(struct netdev *self)
{
    (void)self;
}
...
START_TEST(test_polymorphism)
{
    ...
    hp.base.ops = &unix_socket_ops;
    lb.base.ops = &loopback_ops;
    hp.fd = 1;

    ck_assert_int_eq(a->ops->send(a, pkt, 5), 5);   /* 走 unix_socket_send */
    ck_assert_int_eq(b->ops->send(b, pkt, 5), 5);   /* 走 loopback_send */
    ck_assert_int_eq(memcmp(lb.loop_buf, pkt, 5), 0);
    ck_assert_ptr_eq(a->ops->init, unix_socket_init);
...
START_TEST(test_self_first_arg)
{
    ...
    ck_assert_str_eq(nd->name, "unix_socket_netdev");
    ck_assert_ptr_eq(nd, &hp.base);
}
END_TEST
...
START_TEST(test_container_of)
{
    ...
    n = nd->ops->start(nd);                          /* start 内部也走 container_of */
    ck_assert_int_eq(n, 0);
    ck_assert_int_eq(nd->rx_bytes, 1);
    ck_assert_int_eq(nd->ops->send(nd, buf, (size_t)1), 1);   /* send 用子类 fd 判断 */
}
END_TEST
...
{
    static struct unix_socket_netdev g_hp;
    struct netdev *nd = &g_hp.base;

    g_hp.base.ops = &unix_socket_ops;
